    ...
    void main() {
        ...
        for (int index = 0; index < 5; index++) {
            ...
            try (var forkJoinPool = new ForkJoinPool()) {
                var changePriceTask = new SumPricesTask(products, 1, 0, products.size() - 1, CHUNK_SIZE);
                System.out.println("Starting...");
                long startTime = System.nanoTime();
                
                Double totalSum = forkJoinPool.invoke(changePriceTask);  // invoke zwraca wynik
                
                System.out.println("Total sum: " + NUMBER_FORMAT.format(totalSum));
